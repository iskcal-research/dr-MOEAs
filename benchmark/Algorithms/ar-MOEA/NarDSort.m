function [FrontNo,MaxFNo, phi] = NarDSort(PopObj,nSort,Points,W,delta, epsilon)
...
%--------------------------------------------------------------------------
    ...
    for i = 1 : size(Points,1)
        [curFrontNo, phi] = nardsort(PopObj,Points(i,:),W(i,:),delta, epsilon);
        FrontNo = min(FrontNo,curFrontNo);
    end
    MaxFNo = find(cumsum(hist(FrontNo,1:max(FrontNo)))>=min(nSort,length(FrontNo)),1);
    FrontNo(FrontNo>MaxFNo) = inf;
end

function [FrontNo, phi] = nardsort(PopObj,g,w,delta, eplison)
% Sort the population according to one preferred point
    ...
    phi = Calphi(PopObj, g, w, delta, eplison);
