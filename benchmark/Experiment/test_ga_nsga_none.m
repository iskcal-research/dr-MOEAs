    ...
    for pro = 5:length(problems)
        ...
        for ri = 1:3
            ...
            if (op_debug)
                ...
                Global = GLOBAL('-algorithm', @ghataNS, '-problem', problems{pro}, '-operator', @EAreal, '-N', pop_size,  '-evaluation', (eNum*tao_t+pre)*pop_size, '-mode', 3, '-ghataNS_parameter', {ri, s, tao_t, pre, 0, 0, 0});
                Global.Start();
...
%     labindex = 3;
            ...
            dlmwrite('./Data/Result/EX2/ga_none.csv', data);
            dlmwrite('./Data/Result/EX2/ga_none_details.csv', details);
            dlmwrite(sprintf('./Data/Result/EX2/P%d-R%d', pro, ri), igd_arr_each);
        end
    end
end