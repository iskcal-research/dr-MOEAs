    ...
    for pro = 5:length(problems)
        ...
        for ri = 1:3
            ...
            if (op_debug)
                ...
                Global = GLOBAL('-algorithm', @iRVEA, '-problem', problems{pro}, '-operator', @CSAreal, '-N', pop_size,  '-evaluation', (eNum*tao_t+pre)*pop_size, '-mode', 3, '-iRVEA_parameter', {ri, s, tao_t, pre, 1, 0, 1});
        ...
        %     labindex = 3;
            ...
            dlmwrite('./Data/Result/EX9/wv_ar.csv', data);
            dlmwrite('./Data/Result/EX9/wv_details.csv', details);
            dlmwrite(sprintf('./Data/Result/EX9/P%d-R%d', pro, ri), igd_arr_each);
        end
    end
end