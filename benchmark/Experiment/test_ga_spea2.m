    ...
    for pro = 5:length(problems)
        ...
        for ri = 1:3
            ...
            if (op_debug)
                ...
                Global = GLOBAL('-algorithm', @SPEA2, '-problem', problems{pro}, '-operator', @CSAreal, '-N', pop_size,  '-evaluation', (eNum*tao_t+pre)*pop_size, '-mode', 3, '-SPEA2_parameter', {ri, s, tao_t, pre, 1, 0, 1});
        ...
        %     labindex = 3;
            ...
            dlmwrite('./Data/Result/EX11/spea2.csv', data);
            dlmwrite('./Data/Result/EX11/spea2_details.csv', details);
            dlmwrite(sprintf('./Data/Result/EX11/P%d-R%d', pro, ri), igd_arr_each);
        end
    end
end