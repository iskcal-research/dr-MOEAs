...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        %       obj.Start()
            ...
            if obj.evaluated <= 0
                ...
                obj.PF = obj.problem('PF',obj,pfNum);
                try
                    tic;
                    obj.algorithm(obj);
                catch err
        ...
        function notermination = NotTermination(obj,Population, RefPoint)
        ...
        %       obj.NotTermination(Population)
            ...
            obj.result(index,:) = {obj.evaluated,Population,RefPoint};
    ...
    methods(Access = private, Static)
        ...
        function show(obj)
            if obj.mode == 3 % added
%                 hold on;
%                 plot(obj.PF(:, 1), obj.PF(:, 2), 'o', 'MarkerFaceColor','r');
%                 hold off;
                return;
            end
            clc; fprintf('%s on %s, %d objectives, run %d (%6.2f%%), %.2fs passed...\n',...
