    ...
    for pro = 1:5
        ...
        for ri = 1:3
            ...
            if (op_debug)
                ...
                Global = GLOBAL('-algorithm', @ghataNS, '-problem', @POP, '-operator', @CSAreal, '-N', pop_size,  '-evaluation', (eNum*tao_t+pre)*pop_size, '-mode', 3, '-ghataNS_parameter', {ri, s, tao_t, pre, 1, 0, 1}, '-POP_parameter', {pro});
                Global.Start();
...
%     labindex = 3;
            ...
            dlmwrite('./Data/Result/EX21/csa.csv', data);
            dlmwrite('./Data/Result/EX21/csa_details.csv', details);
            dlmwrite(sprintf('./Data/Result/EX21/P%d-R%d', pro, ri), igd_arr_each);
        end
    end
end