    ...
    for pro = 1:5
        ...
        for ri = 1:3
            ...
            if (op_debug)
                ...
                Global = GLOBAL('-algorithm', @PeEA, '-problem', @POP, '-operator', @CSAreal, '-N', pop_size,  '-evaluation', (eNum*tao_t+pre)*pop_size, '-mode', 3, '-PeEA_parameter', {ri, s, tao_t, pre, 1, 0, 1}, '-POP_parameter', {pro});
        ...
        %     labindex = 3;
            ...
            dlmwrite('./Data/Result/EX23/wv_ar.csv', data);
            dlmwrite('./Data/Result/EX23/wv_details.csv', details);
            dlmwrite(sprintf('./Data/Result/EX23/P%d-R%d', pro, ri), igd_arr_each);
        end
    end
end